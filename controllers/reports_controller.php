...
class ReportsController {
    ...
    public function getMemberStatistics($period = '1_year') {
        try {
            ...
            // Active members with contributions
            $stmt = $this->pdo->prepare("
                SELECT 
                ...
                INNER JOIN (
                    ...
                    GROUP BY member_id
                ) member_totals ON c.member_id = member_totals.member_id
                WHERE c.contribution_date >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)
            ");
            $stmt->execute();
    ...
    public function getContributionPerformance($period = '1_year') {
        try {
            $date_condition = $this->getPeriodCondition($period, 'contribution_date');
            ...
            $stmt = $this->pdo->prepare("
                ...
                FROM contributions 
                WHERE $date_condition AND status = 'Confirmed'
                GROUP BY DATE_FORMAT(contribution_date, '%Y-%m'), contribution_type
                ORDER BY month, contribution_type
            ");
            $stmt->execute();
            ...
            // Target achievement analysis (if targets exist)
            $stmt = $this->pdo->prepare("
                SELECT 
                    target_type,
            ...
            $stmt = $this->pdo->prepare("
                ...
                FROM members m
                INNER JOIN contributions c ON m.member_id = c.member_id
                WHERE c.contribution_date >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)
        ...
        try {
            $stmt = $this->pdo->prepare("
                (SELECT 
                    ...
                    CONCAT(m.first_name, ' ', m.last_name) as member_name,
                    c.amount,
                    c.contribution_type as description,
                    c.contribution_date as transaction_date,
                    c.status
                FROM contributions c
