-- Enhanced Cooperative Society Database Schema
...
CREATE TABLE IF NOT EXISTS share_capital (
    ...
    CONSTRAINT chk_amount_paid_valid CHECK (amount_paid >= 0 AND amount_paid <= total_value)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS notification_queue (
    ...
    recipient_type ENUM('member', 'user', 'role', 'all_members') NOT NULL,
    recipient_id INT NULL COMMENT 'Member ID or User ID',
...
ALTER TABLE loans 
ADD INDEX IF NOT EXISTS idx_loans_status_amount (status, amount),
ADD INDEX IF NOT EXISTS idx_loans_member_status (member_id, status),
ADD INDEX IF NOT EXISTS idx_loans_dates (application_date, approval_date, disbursement_date),
ADD INDEX IF NOT EXISTS idx_loans_next_payment (next_payment_date);

ALTER TABLE contributions 
ADD INDEX IF NOT EXISTS idx_contributions_type_status (contribution_type, status),
ADD INDEX IF NOT EXISTS idx_contributions_member_type (member_id, contribution_type),
ADD INDEX IF NOT EXISTS idx_contributions_date_type (contribution_date, contribution_type),
ADD INDEX IF NOT EXISTS idx_contributions_amount (amount);

-- Add constraints to existing tables
ALTER TABLE loans 
ADD CONSTRAINT IF NOT EXISTS chk_loans_amount_positive CHECK (amount > 0),
ADD CONSTRAINT IF NOT EXISTS chk_loans_interest_rate_valid CHECK (interest_rate >= 0 AND interest_rate <= 100),
ADD CONSTRAINT IF NOT EXISTS chk_loans_term_positive CHECK (term > 0);

ALTER TABLE contributions 
ADD CONSTRAINT IF NOT EXISTS chk_contributions_amount_positive CHECK (amount > 0);

-- ===================================================================
-- TRIGGERS FOR AUTOMATIC CALCULATIONS
-- ===================================================================

-- Trigger to update contribution targets when contributions are made
DELIMITER $$
CREATE TRIGGER IF NOT EXISTS update_contribution_targets_after_contribution
AFTER INSERT ON contributions
FOR EACH ROW
BEGIN
    UPDATE contribution_targets 
    SET amount_achieved = (
        SELECT COALESCE(SUM(c.amount), 0)
        FROM contributions c 
        WHERE c.member_id = NEW.member_id 
        AND c.contribution_type = NEW.contribution_type
        AND c.contribution_date BETWEEN contribution_targets.target_period_start AND contribution_targets.target_period_end
        AND c.status = 'Confirmed'
    ),
    achievement_status = CASE 
        WHEN amount_achieved >= target_amount THEN 'achieved'
        WHEN amount_achieved > 0 THEN 'in_progress'
        ELSE 'not_started'
    END
    WHERE member_id = NEW.member_id 
    AND target_type = NEW.contribution_type
    AND NEW.contribution_date BETWEEN target_period_start AND target_period_end
    AND status = 'active';
END$$
DELIMITER ;

-- Trigger to create audit trail for loan status changes
DELIMITER $$
CREATE TRIGGER IF NOT EXISTS audit_loan_status_changes
...
BEGIN
    IF OLD.status != NEW.status THEN
        INSERT INTO financial_audit_trail (
            ...
            old_values, new_values, amount_involved, performed_by, reason
        ) VALUES (
            'loan', NEW.loan_id, NEW.member_id, 'update',
...
-- Create default share types for the cooperative
INSERT IGNORE INTO share_capital (member_id, share_type, number_of_shares, par_value, purchase_date, status) 
SELECT 1, 'ordinary', 1, 100.00, '2024-01-01', 'active' 
WHERE NOT EXISTS (SELECT 1 FROM share_capital LIMIT 1);

-- Set user variable for audit triggers (this would be set by the application)
