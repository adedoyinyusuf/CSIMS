<?php
/**
 * CSIMS Enhanced Schema Migration Script - PHP Version
 * 
 * This script applies the enhanced cooperative schema using individual PHP statements
 */

require_once __DIR__ . '/config/database.php';

echo "=== CSIMS Enhanced Schema Migration (PHP Version) ===\n\n";
...
$checkMigration = $conn->query("SELECT * FROM schema_migrations WHERE migration_name = 'enhanced_cooperative_schema_php'");
if ($checkMigration && $checkMigration->num_rows > 0) {
    echo "⚠️  Migration 'enhanced_cooperative_schema_php' already applied.\n";
...
try {
    ...
    $statements = [
        ...
        [
            'name' => 'Create loan_collateral table',
            'sql' => "CREATE TABLE IF NOT EXISTS loan_collateral (
        ...
        [
            'name' => 'Create loan_payment_schedule table',
            'sql' => "CREATE TABLE IF NOT EXISTS loan_payment_schedule (
        ...
        // Loan Penalty Config Table
        [
            'name' => 'Create loan_penalty_config table',
            'sql' => "CREATE TABLE IF NOT EXISTS loan_penalty_config (
        ...
        [
            'name' => 'Create contribution_targets table',
            'sql' => "CREATE TABLE IF NOT EXISTS contribution_targets (
        ...
        [
            'name' => 'Create contribution_withdrawals table',
            'sql' => "CREATE TABLE IF NOT EXISTS contribution_withdrawals (
                withdrawal_id INT AUTO_INCREMENT PRIMARY KEY,
                member_id INT NOT NULL,
                withdrawal_type ENUM('partial', 'emergency', 'resignation', 'loan_offset', 'investment') NOT NULL,
                amount DECIMAL(15,2) NOT NULL,
                contribution_types TEXT COMMENT 'JSON array of contribution types',
        ...
        // Share Capital Table
        [
            'name' => 'Create share_capital table',
            'sql' => "CREATE TABLE IF NOT EXISTS share_capital (
                ...
                CONSTRAINT chk_amount_paid_valid CHECK (amount_paid >= 0)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
        ],
        
        // Dividend Declarations Table
        [
            'name' => 'Create dividend_declarations table',
            'sql' => "CREATE TABLE IF NOT EXISTS dividend_declarations (
        ...
        // Member Dividend Payments Table
        [
            'name' => 'Create member_dividend_payments table',
            'sql' => "CREATE TABLE IF NOT EXISTS member_dividend_payments (
        ...
        [
            'name' => 'Create financial_audit_trail table',
            'sql' => "CREATE TABLE IF NOT EXISTS financial_audit_trail (
                ...
                old_values TEXT COMMENT 'Previous values before change',
        ...
        [
            'name' => 'Create workflow_approvals table',
            'sql' => "CREATE TABLE IF NOT EXISTS workflow_approvals (
                ...
                approval_chain TEXT COMMENT 'JSON array of approver user IDs and roles',
        ...
        [
            'name' => 'Create notification_queue table',
            'sql' => "CREATE TABLE IF NOT EXISTS notification_queue (
                ...
                data_payload TEXT COMMENT 'JSON structured data',
                delivery_methods TEXT COMMENT 'JSON array of delivery methods',
                priority ENUM('low', 'normal', 'high', 'urgent') DEFAULT 'normal',
    ...
    foreach ($statements as $index => $statement) {
        ...
        echo "[$statementNumber] {$statement['name']}\n";
        
        $result = $conn->query($statement['sql']);
        ...
        if ($result === false) {
            if (strpos($conn->error, 'already exists') !== false) {
                ...
                throw new Exception("SQL Error in {$statement['name']}: " . $conn->error);
            }
        } else {
            echo "   ✓ Success\n";
            $successCount++;
    ...
    // Insert default data
    echo "\n[" . (count($statements) + 1) . "] Insert default penalty configuration\n";
    $penaltyConfigSql = "INSERT IGNORE INTO loan_penalty_config (
    ...
    if ($conn->query($penaltyConfigSql)) {
        echo "   ✓ Default penalty configuration inserted\n";
        $successCount++;
    } else {
        echo "   ⚠️  Penalty config already exists or error: " . $conn->error . "\n";
        $skipCount++;
    }
    
    // Insert sample share capital data
    echo "\n[" . (count($statements) + 2) . "] Insert sample share capital data\n";
    $shareCapitalSql = "INSERT IGNORE INTO share_capital (member_id, share_type, number_of_shares, par_value, total_value, purchase_date, status)
    SELECT member_id, 'ordinary', 10, 100.00, 1000.00, join_date, 'active'
    FROM members 
    WHERE status = 'Active' 
    LIMIT 5";
    
    if ($conn->query($shareCapitalSql)) {
        $affected = $conn->affected_rows;
        echo "   ✓ Sample share capital data inserted ($affected records)\n";
        $successCount++;
    } else {
        echo "   ⚠️  Share capital data already exists or error: " . $conn->error . "\n";
        $skipCount++;
    }
    
    // Record successful migration
    $conn->query("INSERT IGNORE INTO schema_migrations (migration_name, status, notes) 
                  VALUES ('enhanced_cooperative_schema_php', 'success', 
    ...
    $conn->query("INSERT IGNORE INTO schema_migrations (migration_name, status, notes) 
                  VALUES ('enhanced_cooperative_schema_php', 'failed', '" . $conn->real_escape_string($e->getMessage()) . "')");
