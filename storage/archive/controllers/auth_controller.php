...
class AuthController {
    ...
    public function __construct() {
        ...
        try {
            ...
            // Fallback to a simple security controller to avoid null dereferences
            $this->securityController = new SimpleSecurityController();
        }
    }
...
class SimpleSecurityController {
    public function logSecurityEvent($eventType, $message, $userId = null, $ip = null, $severity = 'low') {
        // Minimal logging to PHP error log to avoid null object issues
        $entry = sprintf('[%s] event=%s severity=%s user_id=%s ip=%s message=%s',
            date('Y-m-d H:i:s'),
            $eventType,
            $severity,
    ...
    public function checkSuspiciousActivity($username, $ip) {
        // Minimal implementation: never lock, just return safe status
        return ['status' => 'ok'];
    }
}
