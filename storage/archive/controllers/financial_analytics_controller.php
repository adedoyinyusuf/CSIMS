...
class FinancialAnalyticsController {
    ...
    private function getFinancialOverview($date_filter) {
        $investmentsExists = $this->tableExists('investments');
        $loanRepExists = $this->tableExists('loan_repayments');
    
        $activeInvestmentsExpr = $investmentsExists
            ? "(SELECT COALESCE(SUM(amount), 0) FROM investments WHERE status = 'Active' AND {$date_filter['where']})"
            : "0";
    
        $outstandingExpr = $loanRepExists
            ? "(SELECT COALESCE(SUM(amount - COALESCE((SELECT SUM(amount) FROM loan_repayments WHERE loan_id = loans.loan_id), 0)), 0) FROM loans WHERE status IN ('Approved', 'Disbursed') AND {$date_filter['where']})"
            : "(SELECT COALESCE(SUM(amount), 0) FROM loans WHERE status IN ('Approved', 'Disbursed') AND {$date_filter['where']})";
    
        $sql = "SELECT 
                    (SELECT COALESCE(SUM(st.amount), 0) FROM savings_transactions st WHERE st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' AND {$date_filter['where']}) as total_contributions,
                    {$activeInvestmentsExpr} as active_investments,
                    {$outstandingExpr} as outstanding_loans,
                    (SELECT COALESCE(SUM(st.amount), 0) FROM savings_transactions st WHERE st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' AND {$date_filter['where']}) -
                    ...
                    (SELECT COALESCE(AVG(st.amount), 0) FROM savings_transactions st WHERE st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' AND {$date_filter['where']}) as avg_contribution";
        ...
        $total_assets = $data['total_contributions'] + $data['active_investments'];
        $loan_to_asset_ratio = $total_assets > 0 ? ($data['outstanding_loans'] / $total_assets) * 100 : 0;
    ...
    private function getCashFlowAnalysis($date_filter) {
        ...
        // Cash inflows
        $inflow_parts = [];
        $inflow_parts[] = "SELECT 'Savings' as source, COALESCE(SUM(st.amount), 0) as amount, COUNT(*) as transaction_count FROM savings_transactions st WHERE st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' AND {$date_filter['where']}";
        if ($loanRepExists) {
            $inflow_parts[] = "SELECT 'Loan Payments' as source, COALESCE(SUM(amount), 0) as amount, COUNT(*) as transaction_count FROM loan_repayments WHERE {$date_filter['where']}";
        }
        if ($investmentsExists) {
            $inflow_parts[] = "SELECT 'Investment Returns' as source, COALESCE(SUM(expected_return), 0) as amount, COUNT(*) as transaction_count FROM investments WHERE status = 'Matured' AND {$date_filter['where']}";
        }
        $inflow_sql = implode(" UNION ALL ", $inflow_parts);
        ...
        // Cash outflows
        $outflow_parts = [];
        $outflow_parts[] = "SELECT 'Loan Disbursements' as source, COALESCE(SUM(amount), 0) as amount, COUNT(*) as transaction_count FROM loans WHERE status IN ('Approved', 'Disbursed', 'Paid') AND {$date_filter['where']}";
        if ($investmentsExists) {
            $outflow_parts[] = "SELECT 'New Investments' as source, COALESCE(SUM(amount), 0) as amount, COUNT(*) as transaction_count FROM investments WHERE {$date_filter['where']}";
        }
        $outflow_sql = implode(" UNION ALL ", $outflow_parts);
        $stmt = $this->conn->prepare($outflow_sql);
        $stmt->execute();
        $outflows = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt->close();
    
        // Totals and ratios expected by the view
        $total_inflow = (float) array_sum(array_map(function($r){ return (float)($r['amount'] ?? 0); }, $inflows));
        $total_outflow = (float) array_sum(array_map(function($r){ return (float)($r['amount'] ?? 0); }, $outflows));
        $net_cash_flow = $total_inflow - $total_outflow;
        $cash_flow_ratio = $total_outflow > 0 ? ($total_inflow / $total_outflow) : 0.0;
    ...
    private function getInvestmentReturns($date_filter) {
        if (!$this->tableExists('investments')) {
            return [
                'total_investments' => 0,
                'total_invested' => 0,
    ...
    private function getMemberFinancialHealth() {
        $sql = "SELECT 
                    ...
                    CASE 
                        ...
                        ELSE LEAST(100, (COALESCE(SUM(st.amount), 0) / COALESCE(SUM(l.amount), 1)) * 50 + 
                                       (COALESCE(SUM(lp.amount), 0) / COALESCE(SUM(l.amount), 1)) * 50)
                    END as financial_health_score
                    
                FROM members m
                LEFT JOIN savings_transactions st ON m.member_id = st.member_id AND st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed'
    ...
    private function getFinancialTrends($period) {
        ...
        $parts[] = "SELECT 
                ...
                WHERE transaction_date >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH) AND transaction_type = 'Deposit' AND transaction_status = 'Completed'
