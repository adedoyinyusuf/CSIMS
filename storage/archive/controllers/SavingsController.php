...
class SavingsController
{
    ...
    private function listAccounts()
    {
        ...
        $accounts = $accountRepository->findAll($filters, ['opening_date' => 'DESC'], $limit, $offset);
     ...
     */
    public function createAccount(?int $memberId = null, ?string $accountType = null, ?float $initialDeposit = null, ?float $interestRate = null)
    {
        $this->requirePermission('create_savings_account');
        
        // If called with parameters (wrapper usage), build data from args
        if ($memberId !== null && $accountType !== null && $interestRate !== null) {
            // Normalize account type input
            $typeMap = [
                'regular' => 'Regular',
                'fixed' => 'Fixed',
                'target' => 'Target',
            ];
            $normalizedType = $typeMap[strtolower($accountType)] ?? $accountType;
            
            $data = [
                'member_id' => (int)$memberId,
                'account_type' => $normalizedType,
                'account_name' => ucfirst($normalizedType) . ' Savings',
                'opening_balance' => (float)($initialDeposit ?? 0),
                'minimum_balance' => 0.0,
            ...
            try {
                ...
                // For wrapper usage, do not echo/redirect; return success status
                return (bool)$account;
            } catch (Exception $e) {
                $this->handleError($e);
                return false;
        ...
        // Fallback to legacy POST-driven handling
        $data = [
    ...
    /**
     * Public wrapper: deposit
     */
    public function deposit(int $accountId, float $amount, int $memberId, string $description = '')
    {
        $this->requirePermission('process_savings_deposit');
        try {
            $processedBy = $_SESSION['admin_id'] ?? $_SESSION['user_id'];
    ...
    /**
     * Public wrapper: withdraw
     */
    public function withdraw(int $accountId, float $amount, int $memberId, string $description = '')
    {
        $this->requirePermission('process_savings_withdrawal');
        try {
            $processedBy = $_SESSION['admin_id'] ?? $_SESSION['user_id'];
     ...
     */
    public function calculateInterest(?int $accountId = null)
    {
        $this->requirePermission('calculate_interest');
        
        // Determine target account ID from parameter or POST
        $targetAccountId = $accountId ?? (!empty($_POST['account_id']) ? (int)$_POST['account_id'] : null);
        
        try {
            $results = $this->savingsService->calculateInterest($targetAccountId);
        } catch (Exception $e) {
            $this->handleError($e);
            return false;
        }
        ...
        // Wrapper usage: return boolean without echoing JSON
        return true;
    }
    ...
    public function getAllAccounts(string $search = '', string $accountTypeFilter = '', string $statusFilter = ''): array
    {
        $accountRepository = new SavingsAccountRepository($this->connection);
        $memberRepository = new MemberRepository($this->connection);
        ...
        if ($accountTypeFilter !== '') {
            $filters['account_type'] = $typeMap[strtolower($accountTypeFilter)] ?? $accountTypeFilter;
        }
        if ($statusFilter !== '') {
            $filters['account_status'] = $statusMap[strtolower($statusFilter)] ?? $statusFilter;
        }

        // Fetch accounts
        $accounts = $accountRepository->findAll($filters, ['opening_date' => 'DESC']);
        ...
        foreach ($accounts as $account) {
            ...
            $memberName = 'N/A';
            try {
                $member = $memberRepository->find($account->getMemberId());
                /** @var \CSIMS\Models\Member|null $member */
                if ($member instanceof \CSIMS\Models\Member) {
                    $memberName = $member->getFullName();
                }
            } catch (Exception $e) {
                // If member lookup fails, continue with placeholder name
            }

            $accountNumber = $account->getAccountNumber();
            $createdAt = $account->getOpeningDate();
            ...
            // Apply search filter (by member name or account number) if provided
            if ($search !== '') {
                $needle = strtolower($search);
                $hayName = strtolower($row['member_name']);
                $hayAccNum = strtolower($row['account_number']);
                if (strpos($hayName, $needle) === false && strpos($hayAccNum, $needle) === false) {
                    continue;
                }
            }
    ...
    {
        // Use unified KPI helper to ensure consistency across dashboards and pages
        if (class_exists('Utilities')) {
            $kpi = Utilities::getUnifiedSavingsKPIs($this->connection);
            return [
                'total_accounts' => (int)($kpi['total_accounts'] ?? 0),
                'total_balance' => (float)($kpi['total_savings_balance'] ?? 0),
                // Preserve legacy key naming for compatibility
                'total_interest' => (float)($kpi['deposits_this_month'] ?? 0),
                'active_members' => (int)($kpi['active_members'] ?? 0),
            ];
        }

        // Fallback to repository-based computation if Utilities is unavailable
        $accountRepository = new SavingsAccountRepository($this->connection);
        $transactionRepository = new SavingsTransactionRepository($this->connection);

        $totalAccounts = $accountRepository->countTotal();
        $stats = $accountRepository->getAccountStatistics();
        $totalBalance = 0.0;
        foreach ($stats as $s) { $totalBalance += (float)($s['total_balance'] ?? 0); }

        $dateFrom = date('Y-m-01');
        $dateTo = date('Y-m-t');
        $txStats = $transactionRepository->getTransactionStatistics([
            'date_from' => $dateFrom,
            'date_to' => $dateTo,
            'transaction_type' => 'Deposit',
        ]);
        $totalDepositsThisMonth = 0.0;
        foreach ($txStats as $row) {
            if (($row['transaction_type'] ?? '') === 'Deposit' && ($row['transaction_status'] ?? '') === 'Completed') {
                $totalDepositsThisMonth += (float)($row['total_amount'] ?? 0);
            }
        }

        $activeMembers = 0;
        $sql = "SELECT COUNT(DISTINCT member_id) AS cnt FROM savings_accounts WHERE account_status = 'Active'";
        if ($result = $this->connection->query($sql)) {
            if ($row = $result->fetch_assoc()) { $activeMembers = (int)($row['cnt'] ?? 0); }
        }

        return [
            'total_accounts' => (int)$totalAccounts,
            'total_balance' => (float)$totalBalance,
            'total_interest' => (float)$totalDepositsThisMonth,
