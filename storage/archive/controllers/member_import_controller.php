...
// Function to detect member_types PK and name columns dynamically
function getMemberTypeColumns(PDO $db) {
    $cols = [];
    $stmt = $db->query("SHOW COLUMNS FROM member_types");
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $cols[] = $row['Field'];
    }
    $pkCandidates = ['type_id', 'id'];
    $nameCandidates = ['type_name', 'name'];
    $pkCol = null;
    $nameCol = null;
    foreach ($pkCandidates as $c) { if (in_array($c, $cols, true)) { $pkCol = $c; break; } }
    foreach ($nameCandidates as $c) { if (in_array($c, $cols, true)) { $nameCol = $c; break; } }
    // Fallbacks
    if (!$pkCol) { $pkCol = 'id'; }
    if (!$nameCol) { $nameCol = 'name'; }
    return ['pk' => $pkCol, 'name' => $nameCol];
}

// Resolve or create a member type; returns [label, id]
function resolveMemberType(PDO $db, ?string $label, $id) {
    $cols = getMemberTypeColumns($db);
    $pk = $cols['pk'];
    $name = $cols['name'];
    $resolvedLabel = null;
    $resolvedId = null;

    // If ID provided, try to fetch its label
    if (!empty($id)) {
        $stmt = $db->prepare("SELECT $pk, $name FROM member_types WHERE $pk = ?");
        $stmt->execute([$id]);
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $resolvedId = $row[$pk];
            $resolvedLabel = $row[$name];
            // If a label is given and differs, keep provided label for members table display
            if (!empty($label) && $label !== $resolvedLabel) {
                $resolvedLabel = $label;
            }
            return [$resolvedLabel, $resolvedId];
        }
        // If ID not found but label provided, fall through to create/find by label
    }

    // If label provided, find or create
    if (!empty($label)) {
        $stmt = $db->prepare("SELECT $pk FROM member_types WHERE $name = ?");
        $stmt->execute([$label]);
        $foundId = $stmt->fetchColumn();
        if ($foundId) {
            return [$label, (int)$foundId];
        }
        // Create new type
        $insert = $db->prepare("INSERT INTO member_types ($name) VALUES (?)");
        $insert->execute([$label]);
        // Try to get last insert id; if not available, refetch
        $newId = $db->lastInsertId();
        if (!$newId) {
            $stmt2 = $db->prepare("SELECT $pk FROM member_types WHERE $name = ?");
            $stmt2->execute([$label]);
            $newId = $stmt2->fetchColumn();
        }
        return [$label, $newId ? (int)$newId : null];
    }

    // Neither label nor valid id provided
    return [null, null];
}
...
try {
    ...
    $optionalHeaders = ['phone', 'gender', 'date_of_birth', 'address', 'membership_type_id', 'ippis_no', 'username', 'password', 'member_type', 'member_type_id'];
    ...
    while (($row = fgetcsv($csvFile)) !== FALSE) {
        ...
        $stmt = $db->prepare("SELECT member_id, first_name, last_name, phone, gender, dob, address, membership_type_id, member_type, ippis_no, username FROM members WHERE email = ?");
        ...
        try {
            if ($existingMember) {
                ...
                if ($importMode === 'insert_only') {
                    ...
                    // Resolve member_type and member_type_id from CSV (if provided)
                    [$resolvedMemberTypeLabel, $resolvedMemberTypeId] = resolveMemberType(
                        $db,
                        $memberData['member_type'] ?? null,
                        $memberData['member_type_id'] ?? null
                    );
                    ...
                    // Apply member_type/member_type_id updates if provided/resolved
                    if (!is_null($resolvedMemberTypeLabel) && $resolvedMemberTypeLabel !== $existingMember['member_type']) {
                        $updateFields[] = "member_type = ?";
                        $updateValues[] = $resolvedMemberTypeLabel;
                    }
                    if (!is_null($resolvedMemberTypeId) && $resolvedMemberTypeId !== (int)($existingMember['member_type_id'] ?? 0)) {
                        $updateFields[] = "member_type_id = ?";
                        $updateValues[] = $resolvedMemberTypeId;
                    }
                ...
                // Resolve member_type and member_type_id from CSV (optional)
                [$resolvedMemberTypeLabel, $resolvedMemberTypeId] = resolveMemberType(
                    $db,
                    $memberData['member_type'] ?? null,
                    $memberData['member_type_id'] ?? null
    ...
    if ($successCount > 0 || $updateCount > 0) {
        ...
        if ($sendCredentials && !empty($newCredentials)) {
            ...
            foreach ($newCredentials as $credential) {
                ...
                try {
                    $emailService->send($credential['email'], $subject, $message);
                } catch (Exception $e) {
