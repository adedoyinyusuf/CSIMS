...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_application'])) {
    try {
        ...
        if ($workflowTemplate) {
            ...
            $workflowId = $workflowService->startWorkflow('loan', $application_id, $amount_requested, $_SESSION['user_id'] ?? null);
            
            // Send notification to member
            $notificationService->sendLoanApplicationConfirmation($member_id, $application_id, $amount_requested);
            
            // Send notification to first approver
            $firstApprover = $workflowService->getCurrentApprover($workflowId);
            if ($firstApprover) {
                $workflowDetails = $workflowService->getWorkflowById($workflowId);
                $notificationService->sendApprovalRequest($firstApprover['email'], $firstApprover['username'], $workflowDetails, $firstApprover['level_number']);
            }
