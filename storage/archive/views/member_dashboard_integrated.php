...
try {
    $stmt = $pdo->prepare("\n        SELECT \n            COALESCE(SUM(CASE WHEN sa.account_type = 'Mandatory' AND st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' THEN st.amount ELSE 0 END), 0) as mandatory_savings,\n            COALESCE(SUM(CASE WHEN sa.account_type = 'Voluntary' AND st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' THEN st.amount ELSE 0 END), 0) as voluntary_savings,\n            COALESCE(SUM(CASE WHEN st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed' THEN st.amount ELSE 0 END), 0) as total_savings,\n            COUNT(DISTINCT DATE_FORMAT(st.transaction_date, '%Y-%m')) as contribution_months\n        FROM savings_transactions st\n        INNER JOIN savings_accounts sa ON st.account_id = sa.account_id\n        WHERE st.member_id = ?\n          AND st.transaction_date >= DATE_SUB(NOW(), INTERVAL 12 MONTH)\n    ");
...
try {
    $stmt = $pdo->prepare("
        (SELECT 'savings_deposit' as type, st.amount, st.transaction_date as created_at, CONCAT('Savings ', sa.account_type, ' deposit') as description\n         FROM savings_transactions st\n         JOIN savings_accounts sa ON st.account_id = sa.account_id\n         WHERE st.member_id = ? AND st.transaction_type = 'Deposit' AND st.transaction_status = 'Completed'\n         ORDER BY st.transaction_date DESC LIMIT 3)\n        UNION ALL\n        (SELECT 'loan_payment' as type, lr.amount as amount, lr.payment_date as created_at, 'Loan payment' as description\n         FROM loan_repayments lr\n         JOIN loans l ON lr.loan_id = l.loan_id\n         WHERE l.member_id = ?\n         ORDER BY lr.payment_date DESC LIMIT 3)\n        ORDER BY created_at DESC LIMIT 5\n    ");
